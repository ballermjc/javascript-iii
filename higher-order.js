...
let ordersTotal = orders.map(order => order["price"] * order[1 + "tax"]);
...
let bobsTotal = purchases.reduce((accumulator, currentValue, index, array) => {
    return currentValue.owner === 'Bob' ? accumulator + currentValue.price : accumulator
}, 0)
